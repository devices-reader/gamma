...
begin
  with frmMain do begin
    ...
    AddTerminalTime('// принято ' + IntToStr(cwIn) + ' байт (доступно ' + IntToStr(Length(sCurr)) + ' из ' + IntToStr(cwMax) + ' байт)',clGray);
...
begin
  ...
  if fAdmin then InfBox('Программа запущена в режиме администратора');
 ...
 {   try
  ...
  with stgDigitals.Rows[0] do begin
    Strings[digNUMBER]  := ' № ';
    Strings[digPORT]    := ' Порт';
    Strings[digPHONE]   := ' Телефон';
    Strings[digDEVICE]  := ' Устройство';
    Strings[digADDRESS] := ' Адрес';
    Strings[digLINE]    := ' Канал';

    Strings[digSHORT]   := '';

    Strings[digTRANS]   := ' К трансф.';
    Strings[digPULSE]   := ' К преобр.';
    Strings[digLOSSE]   := ' К потерь,%';
    Strings[digCOUNT]   := ' Счётчики';
    Strings[digLEVEL]   := ' К пропорц.';
  end;
...
begin
  ...
  with stgAddresses do begin
    ...
    Rows[0].Strings[1] := ' Адреса';
    Rows[0].Strings[2] := ' Числовые пароли';
  end;
  ClearAddresses;
  ...
  with stgAddresses2 do begin
    ...
    Rows[0].Strings[1] := ' Адреса';
    Rows[0].Strings[2] := ' Символьные пароли';
  end;
  ClearAddresses2;
...
begin
  ...
  if chbBulk.Checked then
    AddTerminal('// доступно ' + IntToStr(ComPort.InBuffUsed) + ' байт; ' + Time2Str ,clGray);
  ...
  with ComPort do begin
    if system1 = system_esc then begin
      if InBuffUsed >= queQueryEsc.cwIn then begin
        ComTerminal('// приём по количеству байт: ' + IntToStr(InBuffUsed) + ' из ' + IntToStr(queQueryEsc.cwIn));
        PostInputComPort;
      end;
    end
    else begin
...
begin
  inherited;
  stbMain.Panels[panRepeat].Text := ' повтор: ' + IntToStr(cbMaxRepeat);
end;
...
begin
  ...
  with TapiDevice do begin
    AddTerminal('установлено соединение на скорости ' + IntToStr(BPSRate),clGray);
    AddDial('установлено соединение на скорости ' + IntToStr(BPSRate));
...
begin
  ...
  with TapiDevice do begin
    AddTerminal('ошибка: ' + FailureCodeMsg(FailureCode),clGray);
...
begin
  with stgAllTariffs do begin
    ...
    for i := 1 to ColCount do
      Rows[0].Strings[i] := ' Месяц ' + IntToStr(i);
  end;
end;
...
begin
  ...
  with stZone,StringGrid do begin
    ...
    try
      for i := 1 to RowCount - 1 do begin
        ...
        with mpTariffs[i] do begin
          ...
          if (bHour > 24) or ((bMinute mod 30) <> 0) or
             ...
             (bTariff > 4) then
            raise EConvertError.Create('Ошибка при задании тарифов !');

          if (bHour*60 + bMinute <= wT) and
             not((bTariff = 0) and (bHour = 0) and (bMinute = 0)) then
            raise EConvertError.Create('Изломы тарифого графика должны идти по возрастанию времени !');
      ...
      Row := y;
      WrnBox('Тарифный график задан неправильно !');
      Exit;
    end;
  end;
...
begin
  ...
  with pgcTariffs do begin
    if (ActivePage = tbsPow) or (ActivePage = tbsEng) then begin
      ...
      Push(iwAlfa);
      Push(0);
      InitLoad('Чтение раздельных тарифов для мощности', 12-1, crcGetOldPowTariff);
    end
    else begin
      iwAlfa := 0;
      InitPushCRC;
      Push(iwAlfa);
      Push(0);
      InitLoad('Чтение совмещённых тарифов для мощности и энергии', 12-1, crcGetOldPubTariff);
    end;
  end;
end;
...
begin
  ...
  with pgcTariffs do begin
    if (ActivePage = tbsPow) or (ActivePage = tbsEng) then begin
      ...
      InitLoad('Запись раздельных тарифов для мощности', 12-1, acSetAllOldPowTariffs);
    end
    else begin
      iwAlfa := 0;
      SaveTariff(stgAllTariffsPub,iwAlfa,0);
      InitLoad('Запись совмещённых тарифов для мощности и энергии', 12-1, acSetAllOldPubTariffs);
    end;
  end;
{
  with pgcTariffs do begin
    if ActivePage = tbsPow then begin
      ...
      if Application.MessageBox(
           PChar('Записать тарифы для мощности для месяца ' + IntToStr(iwAlfa+1) + ' ?'),
           'Вопрос',mb_IconQuestion + mb_YesNo + mb_DefButton1) = idYes then begin
        SaveTariff(stgAllTariffsPow,iwAlfa,0);
    ...
    if ActivePage = tbsEng then begin
      ...
      if Application.MessageBox(
           PChar('Записать тарифы для энергии для месяца ' + IntToStr(iwAlfa+1) + ' ?'),
           'Вопрос',mb_IconQuestion + mb_YesNo + mb_DefButton1) = idYes then begin
      ...
      if Application.MessageBox(
           PChar('Записать совмещённые тарифы для месяца ' + IntToStr(iwAlfa+1) + ' ?'),
           'Вопрос',mb_IconQuestion + mb_YesNo + mb_DefButton1) = idYes then begin
        SaveTariff(stgAllTariffsPub,iwAlfa,0);
...
begin
  ...
  try
    ...
    ErrBox('Учёт тарифов для мощности и энергии задан неправильно !');
    Exit;
  end;
...
begin
  ...
  with od_CustomLoad do begin
    try
      ...
      ErrBox('Ошибка при чтении списка каналов !');
    end;
  end;
end;
        ...
        Writeln(FOutput,'\\ ' + FormatDateTime('hh.nn.ss dd.mm.yyyy',Now));
{
        ...
        InfBox('Таблица сохранена успешно');
      end;
    except
      ErrBox('Ошибка при сохранении таблицы !')
    end;
  end;
end;
...
begin
  inherited;
  SaveGrid(stgDigitals, 'Список каналов', stSETTINGS, table_canals);
end;
...
begin
  ...
  try
    ...
    ErrBox('Ошибка при вводе данных !');
  end;
...
begin
  ...
  InitLoad('Запись коэффициентов', bCANALS*7, acSetTransEng);
end;
...
begin
  ...
  InitLoad('Чтение коэффициентов', bCANALS*5-1, acGetTransEng);
end;
  ...
  with sd_SaveGrid do begin
    try
      ...
      if Execute then begin
        ...
        Writeln(FOutput,'\\ ' + stName);
        Writeln(FOutput,'\\ дата создания: ' + FormatDateTime('hh.nn.ss dd.mm.yyyy',Now));
        Writeln(FOutput,'');

        SaveTariffGrid(stgTariffsDay, '\\ Тарифы на сутки');

        SaveAllTariffsGrid(stgAllTariffsPow, '\\ Тарифы для мощности');
        SaveAllTariffsGrid(stgAllTariffsEng, '\\ Тарифы для энергии');
        SaveAllTariffsGrid(stgAllTariffsPub, '\\ Тарифы совмещённые');
        ...
        InfBox('Список тарифов сохранён успешно');
      end;
    except
      ErrBox('Ошибка при сохранении списка тарифов !')
    end;
  end;
end;
    ...
    try
      ...
      if Execute then begin
        if ImportTariffs(FileName) then
          InfBox('Список тарифов прочитан успешно');
      end;
    except
      ErrBox('Ошибка при чтении списка тарифов !');
    end;
  end;
end;
...
begin
  inherited;
  InfBox('версия от 24 апреля 2022 года');
end;
...
begin
  ...
  InitPushCRC;
  Push(iwAlfa);
  InitLoad('Чтение групп', bGROUPS-1, acGetGroups);
end;
...
begin
  ...
  SaveGroups(iwAlfa);
  InitLoad('Запись групп', bGROUPS-1, acSetGroups);
end;

procedure TfrmMain.btbExportGroupsClick(Sender: TObject);
begin
  inherited;
  SaveGrid(stgGroups, 'Список групп', stSETTINGS, table_groups);
end;
    ...
    try
      ...
      if Execute then begin
        if ImportGroups(FileName) then
          InfBox('Список групп прочитан успешно');
      end;
    except
      ErrBox('Ошибка при чтении списка групп !');
    end;
  end;
end;
...
begin
  ...
  Push(1);
  Push(iwAlfa);
  InitLoad('Чтение телефонов', bCANALS-1, acGetPhones);
end;
...
begin
  ...
  SavePhones(iwAlfa);
  InitLoad('Запись телефонов', bCANALS-1, acSetPhones);
end;
...
begin
  inherited;
  SaveGrid(stgPhones, 'Список телефонов', stSETTINGS, table_phones);
end;
    ...
    try
      ...
      if Execute then begin
        if ImportPhones(FileName) then
          InfBox('Список телефонов прочитан успешно');
      end;
    except
      ErrBox('Ошибка при чтении списка телефонов !');
    end;
  end;
end;
...
begin
  ...
  Push(iwAlfa div $100);
  Push(iwAlfa mod $100);  
  InitLoad('Чтение параметров', wPARAMS*2-1, acGetDivid);
end;
...
begin
  ...
  InitLoad('Запись параметров', wPARAMS*2-1, acSetDivid);
end;
    ...
    try
      ...
      if Execute then begin
        if ImportParams(FileName) then
          InfBox('Список параметров прочитан успешно');
      end;
    except
      ErrBox('Ошибка при чтении списка параметров !');
    end;
  end;
end;
...
begin
  inherited;
  SaveGrid(stgParams, 'Список параметров', stSETTINGS, table_params);
end;
...
begin
  inherited;
  if (Application.MessageBox(
      PChar('Установить К пропор. в исходные значения ? '),
      'Внимание',mb_IconExclamation + mb_YesNo + mb_DefButton2) <> idYes) then Exit;
...
begin
  ...
  if (rgrEvent.ItemIndex < 6) then begin
    ...
    Push(iwAlfa);
    InitLoad('Чтение событий', bRECORD_PAGES-1, acGetRecords);
  end
  else begin
...
begin
  inherited;
  SaveRich(redRecords, 'События ('+rgrEvent.Items[rgrEvent.ItemIndex] +') ' + FormatDateTime('hh.nn.ss dd.mm.yyyy ',Now));
end;
...
begin
  ...
  Push(28);
  Push(iwAlfa);
  InitLoad('Чтение адресов и паролей', bCANALS-1, acGetAddresses);
end;
...
begin
  ...
  InitLoad('Запись адресов и паролей', bCANALS-1, acSetAddresses);
end;
    ...
    try
      ...
      if Execute then begin
        if ImportAddresses(FileName) then
          InfBox('Список адресов и паролей прочитан успешно');
      end;
    except
      ErrBox('Ошибка при чтении списка адресов и паролей !');
    end;
  end;
end;
...
begin
  inherited;
  SaveGrid(stgAddresses, 'Список адресов и числовых паролей', stSETTINGS, table_addresses);
end;
...
begin
  try
    with lbxRelaxs do begin
       lblRelaxs.Caption := 'Всего праздников: '+IntToStr(Count);       
    ...
    ErrBox('Ошибка при отображении праздников !');
  end
end;
...
begin
  try
    if lbxRelaxs.Count >= bMAXRELAXS then begin WrnBox('Максимальное количество праздников в списке: '+IntToStr(bMAXRELAXS)); exit; end;
    ...
    for i := 1 to lbxRelaxs.Count do begin
      ...
      if (r.tiSelf.bDay = tiT.bDay) and (r.tiSelf.bMonth = tiT.bMonth) then begin WrnBox('Такой праздник уже задан !'); exit; end;
    end;
    ...
    ErrBox('Ошибка при добавлении праздника !');
  end
end;
...
begin
  ...
  try
    ...
    ErrBox('Ошибка при записи праздников !');
  end
end;
...
begin
  ...
  with sd_SaveGrid,lbxRelaxs do begin
    try
      InitialDir := ExtractFileDir(ParamStr(0)) + stSETTINGS;
      SysUtils.ForceDirectories(InitialDir);
      FileName := 'Список праздников' + '.txt';

      if Execute then begin
        AssignFile(FOutput, FileName);
        Rewrite(FOutput);

        Writeln(FOutput,'\\ ' + 'Список праздников');
        Writeln(FOutput,'\\ ' + FormatDateTime('hh.nn.ss dd.mm.yyyy',Now));
        ...
        InfBox('Список праздников сохранен успешно');
      end;
    except
      ErrBox('Ошибка при сохранении списка праздников !')
    end;
  end;
end;
    ...
    try
      ...
      if Execute then begin
        if ImportRelaxs(FileName) then
          InfBox('Список праздников прочитан успешно');
      end;
    except
      ErrBox('Ошибка при чтении списка праздников !');
    end;
  end;
end;
...
begin
  ...
  Push(15);
  Push(iwAlfa);
  InitLoad('Чтение графика опроса счетчиков', 4, acGetGraphInquiry);
end;
...
begin
  ...
  for x := 1 to 4 do begin
    for y := 1 to 48 do begin
      ...
      then begin ErrBox('Не задан столбец таблицы: ' + GetInterval(y-1)); exit; end;
    end;
  end;
  ...
  InitLoad('Запись графика опроса счетчиков', 4, acSetGraphInquiry);
end;
...
begin
  ...
  for y := 1 to 48 do begin
    ...
    else begin ErrBox('Не задан столбец таблицы: ' + GetInterval(y-1)); exit; end;
  end;
  ...
  Push($55);
{
  with rgrGraphRecalc do begin
    ...
    else begin ErrBox('Правило перерачета данных не задано'); exit; end;
  end;
}
  for y := 1 to 48 do begin
...
begin
  ...
  for y := 1 to 48 do begin
    ...
    if s = GRAPH_PORT3
      ...
      begin ErrBox('Не задан столбец таблицы: ' + GetInterval(y-1)); exit; end;
  end;
...
begin
  inherited;
  SaveGrid(stgGraphInquiry, 'График опроса счетчиков', stSETTINGS, table_graph_inquiry);
end;

procedure TfrmMain.btbSaveGraphCorrectClick(Sender: TObject);
begin
  inherited;
  SaveGrid(stgGraphCorrect, 'График коррекции времени', stSETTINGS, table_graph_correct);
end;

procedure TfrmMain.btbSaveGraphRecalcClick(Sender: TObject);
begin
  inherited;
  SaveGrid(stgGraphRecalc, 'График перерасчета данных', stSETTINGS, table_graph_recalc);
end;

procedure TfrmMain.btbSaveGraphTransitClick(Sender: TObject);
begin
  inherited;
  SaveGrid(stgGraphTransit, 'График автоматического транзита', stSETTINGS, table_graph_transit);
end;
    ...
    try
      ...
      if Execute then begin
        if ImportGraphInquiry(FileName) then
          InfBox('График опроса счетчиков прочитан успешно');
      end;
    except
      ErrBox('Ошибка при чтении графика опроса счетчиков !');
    end;
  end;
end;
...
begin
  ...
  with od_CustomLoad do begin
    try
      ...
      ErrBox('Ошибка при чтении графика времени коррекции !');
    end;
  end;
end;
...
begin
  ...
  with od_CustomLoad do begin
    try
      ...
      ErrBox('Ошибка при чтении графика перерасчета данных !');
    end;
  end;
end;
...
begin
  ...
  with od_CustomLoad do begin
    try
      ...
      ErrBox('Ошибка при чтении графика транзита !');
    end;
  end;
end;
...
begin
  ...
  Push(113);
  Push(iwAlfa);
  InitLoad('Чтение названий каналов', bCANALS-1, acGetCanalsNames);
end;
...
begin
  ...
  InitLoad('Запись названий каналов', bCANALS-1, acSetCanalsNames);
end;
    ...
    try
      ...
      if Execute then begin
        if ImportCanalsNames(FileName) then
          InfBox('Список названий каналов прочитан успешно');
      end;
    except
      ErrBox('Ошибка при чтении списка названий каналов !');
    end;
  end;
end;
...
begin
  inherited;
  SaveGrid(stgCanalsNames, 'Список названий каналов', stSETTINGS, table_canals_names);
end;
...
begin
  ...
  Push(115);
  Push(iwAlfa);
  InitLoad('Чтение названий групп', bGROUPS-1, acGetGroupsNames);
end;
...
begin
  ...
  InitLoad('Запись названий групп', bGROUPS-1, acSetGroupsNames);
end;
...
begin
  inherited;
  SaveGrid(stgGroupsNames, 'Список названий групп', stSETTINGS, table_groups_names);
end;
    ...
    try
      ...
      if Execute then begin
        if ImportGroupsNames(FileName) then
          InfBox('Список названий групп прочитан успешно');
      end;
    except
      ErrBox('Ошибка при чтении списка названий групп !');
    end;
  end;
end;
...
begin
  ...
  Push(136);
  Push(iwAlfa);
  InitLoad('Чтение адресов и паролей', bCANALS-1, acGetAddresses2);
end;
...
begin
  ...
  InitLoad('Запись адресов и паролей', bCANALS-1, acSetAddresses2);
end;
...
begin
  inherited;
  SaveGrid(stgAddresses2, 'Список адресов и символьных паролей', stSETTINGS, table_addresses2);
end;
